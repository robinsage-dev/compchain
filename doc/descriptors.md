# Support for Output Descriptors in Compchain Core

Since Compchain Core v0.17, there is support for Output Descriptors in the
...
- [WIF](https://en.compchain.it/wiki/Wallet_import_format) encoded private keys may be specified instead of the corresponding public key, with the same meaning.
...
on Compchain's OP_CHECKMULTISIG opcode. To support these, we introduce the
...
existing Compchain Core wallets, a convenience function `combo` is
