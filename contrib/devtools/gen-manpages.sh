...
COCOVER=($($COMPCHAINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
...
for cmd in $COMPCHAIND $COMPCHAINCLI $COMPCHAINTX $COMPCHAINQT; do
  cmdname="${cmd##*/}"
  help2man -N --version-string=${COCOVER[0]} --include=footer.h2m -o ${MANDIR}/${cmdname}.1 ${cmd}
  sed -i "s/\\\-${COCOVER[1]}//g" ${MANDIR}/${cmdname}.1
done
