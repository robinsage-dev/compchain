...
###   compchaincore.org and compchain.org and compares them.
...
WORKINGDIR="/tmp/compchain_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://compchaincore.org"
HOST2="https://compchain.org"
BASEDIR="/bin/"
VERSIONPREFIX="compchain-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'compchain-',
...
if [ $? -ne 0 ]; then
   echo "compchain.org failed to provide signature file, but compchaincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "compchain.org and compchaincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Compchain Core binary release signing key installed?"
   fi
