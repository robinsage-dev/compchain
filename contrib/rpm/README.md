...
The RPM spec file provided here is for Compchain-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://compchain.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Compchain git repository](https://github.com/compchain/compchain) but are not part of
...
Currently only `Source10` is used, for the example `compchain.conf` file.
...
in the compchain git repository. At present only an SVG version of the Compchain
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the compchain
...
needed when 0.12.1 is released, a proper fix is already in the Compchain git
master branch.
...
idea to build Compchain Core against a static version of that library compiled
...
The `compchain.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `compchain-server` package. At this point, most RPM based
...
The `compchain-server` package should have SELinux support. How to properly do
...
Compchain-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/compchain/). He has been
packaging Compchain for Fedora at least since 2012.
...
4. I renamed the `compchain` package that contains the Qt GUI to `compchain-core` as
that appears to be how the general population refers to it, in contrast to
`compchain-xt` or `compchain-classic`. I wanted to make sure the general population
