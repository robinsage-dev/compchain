#!/bin/bash
# Copyright (c) 2013 The Compchain Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Compchain database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Compchain datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Compchain datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Compchain 0.7 datadir."
    ;;
  3)
    echo "Detected Compchain pre-0.8 datadir."
    ;;
  4)
    echo "Detected Compchain 0.8 datadir."
    ;;
esac
