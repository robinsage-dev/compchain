// Copyright (c) 2016-2018 The Compchain Core developers
...
#if defined(NDEBUG)
# error "Compchain cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Compchain Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s COCO additional fees, %d delta bytes\n",
...
void ThreadScriptCheck() {
    RenameThread("compchain-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Skip checking low difficulty gensis block
    ...
    if (block.GetHash() == chainparams.GetConsensus().hashGenesisBlock) {
        ...
        // return true; // Make coinbase spendable for premine
    }
    ...
    // Special case to allow genesis block coinbase transaction to be spent
    if (block.GetHash() != chainparams.GetConsensus().hashGenesisBlock) {
        assert(pindex->pprev);
    ...
    // Skep for genesis block since it contains only a coinbase tx and 
