 ...
 * between compchaind, and compchain-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("compchain-shutoff");
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", COMPCHAIN_PID_FILENAME), false, OptionsCategory::OPTIONS);
#else
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("compchain-loadblk");
...
/** Sanity checks
 *  Ensure that Compchain is running in a usable environment with all
...
{
    // Make sure only a single Compchain process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", COMPCHAIN_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if compchain is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if compchain is started while in a temporary directory.\n",
