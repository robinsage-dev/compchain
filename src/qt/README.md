This directory contains the CompchainQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Compchain UI.
...
ClientModel is used by the main application `compchaingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/bitcoin/compchain/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a compchain:... link from a browser).
...
* don't change `local/compchain_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "compchain-qt" under "Run", which is where you can also add command line arguments)
